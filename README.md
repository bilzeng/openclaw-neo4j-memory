...
### 3. 核心事实去重与重复对话溯源 (Deduplication & Provenance)
本插件底层巧妙运用了 Neo4j `MERGE` 与 `CREATE` 语句的差异化特性，完美解决了大模型长期对话下“记忆垃圾堆积”的核心痛点：
- **实体与事实关系严格去重 (`MERGE`)**：不管你对智能体强调多少遍相同的客观事实（比如每天重复打字说“张三是后端”），知识图谱内部的认知网络绝不会分裂出冗余节点。图内永远只会保留最纯粹、唯一的 `(张三)-[是]->(后端)` 三角核心链路，确保检索无冗余噪音。
- **对话发生轨迹独立留存 (`CREATE`)**：为了让智能体拥有追溯上下文的时间感知力，你每次说过的“原话文本”都会被作为独立的溯源凭证节点（Intent）创建，并连线挂接在唯一的真值事实旁边。也就是说，核心共识事实只有一个，但每一次反复提及的时间和原话语境都能被安全存证！
